...
const getAvailableRequests = async (userId) => {
    ...
    const userAddedRequestData = await getUserRequestData.fetchAddedRequests(
        userId
    );
    for (let i = 0; i < userAddedRequestData.length; i++) {
        let days = calaculateDays(
            userAddedRequestData[i].startDate,
            userAddedRequestData[i].endDate
        );
    ...
    const availableRequests = [
        {
            ...
            totalLeaves: new Date().getMonth() + 1,
            remainingLeaves: casualLeave,
        },
        {
            leaveType: "Leave without pay",
            totalLeaves: 30,
            remainingLeaves: leaveWithoutPay,
        },
        {
            leaveType: "Restricted Holiday",
            totalLeaves: 5,
            remainingLeaves: restrictedHoliday,
        },
        {
            leaveType: "Work From Home",
            totalLeaves: 5,
            remainingLeaves: workFromHome,
        },
    ];
...
const createRequestNotification = async (userId) => {
    ...
    await db.sequelize.query(
        "EXEC dbo.sp_users_postusernotification :notificationId, :content, :sender, :receiver, :date, :type",
        {
            replacements: {
...
const updateSubordinateRequestNotification = async (
    senderId,
    receiverId,
    status
) => {
        ...
        "EXEC dbo.sp_users_postusernotification :notificationId, :content, :sender, :receiver, :date, :type",
        {
            replacements: {
                ...
                content: `Your request has been ${
                    status.toLowerCase() === "approve" ? "approved" : "rejected"
                } by ${sender.name}.`,
                sender: sender.hrmid,
...
module.exports = {
    ...
    updateSubordinateRequestNotification,
