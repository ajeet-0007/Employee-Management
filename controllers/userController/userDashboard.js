...
exports.getUserDashboard = async (req, res) => {
    ...
    createDashboardData(dashboardData, userSkillsData, "userSkills");

    const userProfileData = await getUserProfileData.fetchProfile(
        req.user.userEmail
    );
    createDashboardData(dashboardData, userProfileData, "userProfile");

    const userAttendanceData = await getUserAttendanceData.fetchAttendance(
        req.user.userEmail
    );
    createDashboardData(dashboardData, userAttendanceData, "userAttendanceList");

    const userRequestData = await getUserRequestData.fetchRequest(
        req.user.userEmail
    );
    createDashboardData(dashboardData, userRequestData, "userRequestList");

    const userProjectListData = await getUserProjectListData.fetchProjectList(
        req.user.userEmail
    );
    createDashboardData(dashboardData, userProjectListData, "userProjectList");
