...
exports.getUserDashboard = async (req, res) => {
	...
	const userSkillsData = await getUserSkillsData.fetchSkills(req.user.userEmail);
	createDashboardData(userSkillsData, 'userSkills', 'skills', dashboardData);

	const userProfileData = await getUserProfileData.fetchProfile(req.user.userEmail);
	createDashboardData(userProfileData, 'userProfile', 'profile', dashboardData);

	const userAttendanceData = await getUserAttendanceData.fetchAttendance(req.user.userEmail);
	createDashboardData(userAttendanceData, 'userAttendance', 'attendance', dashboardData);

	const userRequestData = await getUserRequestData.fetchRequests(req.user.userEmail);
	createDashboardData(userRequestData, 'userRequests', 'requests', dashboardData);
